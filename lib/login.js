...
module.exports = {
    ...
    async _performLoginAsync(url, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, defaultUsername, defaultPassword) {
        ...
        let browser;
        try {
            const args = headless ? [`--incognito`] : [`--incognito`, `--app=${url}`, `--window-size=${WIDTH},${HEIGHT}`];
            ...
            debug("Skip Sign In stage if already signed in");
            try {
                await page.waitForSelector(`input[name=loginfmt].moveOffScreen`, { timeout: 5000 });
            } catch (err) {}
